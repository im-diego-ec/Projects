...
**El tamaño de lo que hoy cada proyecto reinventaría**, medido contra el consumidor de
referencia (solo archivos versionados; el `state` y los `.tfvars` están correctamente
...
`.gitignore` para el `state` y los `.tfvars` —que en el consumidor de referencia ya están
...
**Impacto en los consumidores actuales: ninguno, y es verificable.** proyecto-origen
