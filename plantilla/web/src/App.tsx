...
const RespuestaSalud = z.object({ status: z.string() });
const RespuestaHola = z.object({ message: z.string(), userId: z.string() });
...
async function leerSalud(): Promise<string> {
  ...
  // (el API viejo todavia desplegado, un proxy que devuelve HTML), no una
  // excepcion. Se distingue de "no hay conexion" a proposito: los dos fallos
  // se arreglan en lugares distintos y el mensaje tiene que decir cual es.
  const leido = RespuestaSalud.safeParse(cuerpo);
  return leido.success ? leido.data.status : RESPUESTA_INESPERADA;
}
...
async function leerHola(): Promise<string> {
  ...
  return `${leido.data.message} (userId: ${leido.data.userId})`;
}
      ...
      <div className="w-full max-w-md bg-white border border-neutral-200 rounded-xl p-8 space-y-6">
        <header className="space-y-1">
          <p className="text-sm font-medium text-orange-600">
            ORG · Transformación Digital y Data
          </p>
