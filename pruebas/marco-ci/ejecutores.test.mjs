...
// mover la clase a AVISOS y bajar la fila del backlog de docs/08-reglas-no-escritas.md.
test("corpus generado · el residuo A16 sigue MUDO, y queda fijado en vez de olvidado", () => {
  ...
  assert.deepEqual(
    ...
    [],
    "el check EMPEZO a ver estas entradas, o sea que el residuo A16 se cerro. No es una falla: es la buena noticia. Arreglo: movelas de la clase \"residuo\" a \"aviso\" en generar.mjs y baja su fila del backlog en docs/08-reglas-no-escritas.md",
  );
});
