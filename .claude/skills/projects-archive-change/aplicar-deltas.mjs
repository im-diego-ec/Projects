...
function guardaDeAcople(change) {
  ...
  const deFuera = (buscada.stdout || "")
    ...
    .filter(({ archivo }) => !archivo.startsWith(`${rutaDelChange}/`) && !archivo.startsWith(rutaArchive));
  ...
  for (const { linea } of prosa) console.log(`::warning::mencion en prosa que va a quedar vieja: ${linea}`);
  if (prosa.length > 0) {
    console.log(
