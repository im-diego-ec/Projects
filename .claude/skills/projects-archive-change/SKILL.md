...
# LECTURAS acotadas.
# Lo que ESCRIBE fuera del arbol —git checkout, git pull, git add, git commit, git push, gh pr create y los `npx` del CLI de OpenSpec (que descargan y ejecutan un paquete)— queda deliberadamente afuera: no es que la
...
memoria). **El pin no esta escrito en el repo consumidor**: vive en el `default`
del input `version_openspec` del workflow reusable, y los consumidores lo
heredan sin repetirlo. Se lee **desde un checkout del marco**
(`im-diego-ec/Projects`):

```bash
node .claude/skills/projects-archive-change/aplicar-deltas.mjs \
  --pin-openspec .github/workflows/marco-ci.yml
```

**Verificacion del pin:** tiene que imprimir **solo el numero**, `X.Y.Z`, y ese es
el numero de todos los comandos `npx` de abajo (al 2026-08-24 imprime `1.9.0`).
Si el archivo no es el workflow reusable, el comando sale **1** diciendo que no
pudo leer el pin, en vez de imprimir vacio y dejarte concluir que no hay ninguno.

Si imprime vacio, estas parado en un repo que no es el marco. **No busques el pin
con `git grep -n "@fission-ai/openspec@" -- .github/`**: en el marco devuelve tres
lineas donde la version es una variable de shell (`${VERSION_OPENSPEC}`,
`${PIN}`) y en un repo consumidor **no devuelve ninguna linea**. En los dos casos
te quedas sin numero, y el paso siguiente pasa a ser una adivinanza.
...
npx --yes @fission-ai/openspec@X.Y.Z validate --all --strict
npx --yes @fission-ai/openspec@X.Y.Z list
node actions/guardrail-deltas/check-openspec-deltas.mjs
```
...
- El guardrail de deltas en verde. En este repo el script vive en
  `actions/guardrail-deltas/`; desde un repo consumidor, se corre el mismo script
  desde un checkout del marco.

Los tres verdes son la evidencia de que el archive quedo bien hecho. Pegalos en
el PR.
...
`aplicar-deltas.mjs` vive junto a esta skill, **no en `actions/`**, asi que el
banco de pruebas de las composite actions del CI de Projects **no lo cubre**. Se
verifico a mano el 2026-08-19 sobre fixtures sinteticas, en estos casos:
