...
| 17 | La exencion de este check para lo GENERADO es por extension, no por origen | El paso de ejecutores pinados exime `.md` y `.mdc` por pathspec. Un artefacto generado no es codigo que alguien pueda arreglar —el marco ya tiene esa regla escrita para el formateador— pero la exencion adivina el origen por la extension del archivo. Una superficie de agente nueva que renderice a `.txt` o `.toml` vuelve a caer. Medido el 2026-08-21: `.cursor/rules/00-marco.mdc` puso en rojo a un-proyecto-anterior con la prosa de la propia regla del pin citando `npx --yes openspec` como contraejemplo. La version derivada exige que `actions/constitucion` publique sus rutas como output y que el paso las reciba entre jobs. Se intento filtrar por el SELLO del artefacto y una prueba del banco lo rechazo con razon: «el sello lo valida UNA sola pieza», y dos lugares interpretando el mismo sello es el defecto de las 12 contra 64 posiciones hex de esta semana |
| 22 | Un repositorio desplegable puede no tener su infraestructura decidida, y nada lo dice | Decidido en el change `infra-exigible` y **parcialmente implementado, declarado como tal** (su D8). El andamio ya reparte `infra/` y `infra-prod/` con lo derivable funcionando y con los pendientes de decision —seis en dev, siete en prod—, cada uno con su criterio. Lo que NO existe es la compuerta: los pendientes usan un token propio que el paso de marcadores no cuenta. **El motivo esta medido**: con el marcador gateado del andamio, un repositorio nuevo pasaba de 3 marcadores a 21, y 18 de esos son decisiones que necesitan cuentas, nombres de recursos y verificaciones que nadie puede hacer el primer dia; el paso es fallo duro, asi que arrastraba `ci-ok` y **un repo recien nacido no podia llegar a verde**. Ademas violaba el propio requirement del change, que exige inertness para un repositorio que no se despliega. La compuerta llega con el change del despliegue, que es cuando «este repositorio se despliega» se vuelve verificable — inventar el disparador antes es adivinar la forma de una pieza que todavia no se diseño, y el marco ya pago una semana de un ruleset pidiendo el check equivocado |
| 23 | Que un pendiente del andamio lleve un criterio UTIL no lo puede verificar ningun check | La guarda de `pruebas/andamio/infra-pendientes.test.mjs` verifica que cada pendiente TENGA sus tres partes —que falta, como se decide, que queda sin garantia— y ahi se termina lo automatizable: distinguir un criterio util de una frase que suena bien no es decidible con un escaneo. Es la misma clase de limite que el marco ya tiene escrito para el check del CHANGELOG, que mira que el PR lo toque y no si lo que escribiste es bueno. La salida NO es un check mas: es que la revision ocurra **una vez**, en el PR del change que escribe el pendiente, en vez de una vez por proyecto que lo hereda. Queda como disciplina declarada del review, y su costo real es una linea de checklist para el builder par |
| 21 | El pin del marco y la constitucion regenerada no se mueven juntos | Decidido en **Projects ADR 004** y no implementado. Hoy los mueven dos mecanismos distintos: los pines por el PR de Dependabot, el artefacto por el workflow semanal del andamio — y ese workflow es un **no-op para su propio caso de uso**, medido el 2026-08-22 con el pin en 1.4.1: corre la action de la version que el repo YA tiene pinada, cuyo canonico viaja adentro, asi que regenera el artefacto que ya estaba (salida «version 1.3.0», identica byte por byte, sin cambios y sin PR). El bloqueo es circular: el PR del bump nace ROJO por el artefacto viejo, el workflow corre el pin VIEJO y no puede ayudar, y el pin nuevo —el unico con el que se regenera— vive dentro del PR rojo. El arreglo decidido es que ese mismo workflow mueva el pin Y regenere, y abra UN PR que nazca verde; cuesta convertir `TOKEN_ACTUALIZAR_MARCO` de opcional en REQUERIDO por repo, y saca el bump del marco de la cobertura de Dependabot (el censo pasa al barrido de la organizacion, que no depende del comportamiento de un tercero). Se construye cuando exista el SEGUNDO consumidor: con uno solo, los defectos del workflow se descubren en el unico consumidor que hay. Mientras tanto lo que pasa es el paso manual —bajar el artifact `constitucion-al-dia` que sube el job de marco y commitearlo sobre el PR del bump— y el encabezado del workflow lo declara ahi mismo, con la medicion |
