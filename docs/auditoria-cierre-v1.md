...
proyecto-origen, con el consumidor real como banco de pruebas. Veinte afirmaciones del
...
```
cd proyecto-origen/web && node node_modules/vitest/vitest.mjs run --coverage
...
diario de este marco (la historia de proyecto-origen ya tiene
...
`Ejecutores de paquetes pinados` corrido tal cual contra proyecto-origen:
...
exit 1 y `process.exit(1)`. parqueadero llama al workflow por `@v1` (`ci.yml:55`), asi que lo
...
En `main` de proyecto-origen el eslabon no existe: `git ls-files | grep -i projects` -> exit 1,
...
proyecto-origen. Cuesta doble: manda a revisar a mano algo que el CI caza, y ensena que las
...
4. **Un PR en proyecto-origen, mergeado y verde**, que pine las 5 entradas
...
11. Exencion NO declarada: Projects usa actions de terceros pinadas por TAG MOVIL dentro del workflow
    ...
    `docs/reglas-no-escritas.md` cuenta "Dependabot semanal" entre lo que YA es automatico. La
...
13. `docs/reglas-no-escritas.md` esta desactualizado en las dos direcciones a la vez. La fila 10
...
20. La adopcion tiene el huevo antes que la gallina para un repo que ya existe. `constitucion.mjs`
    ...
    contra un espejo del arbol de parqueadero: sin el archivo, exit 1; con el
...
21. El artefacto generado queda DENTRO del formateador en el consumidor existente.
    `grep -n projects .prettierignore` en parqueadero da exit 1, y
...
8. **No revise cuanto DIVERGIO ya el scaffold en los consumidores reales.** Comparar `plantilla/`
   contra el arbol de proyecto-origen archivo por archivo es el item 1 de la revision
