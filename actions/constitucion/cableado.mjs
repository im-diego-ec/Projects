...
export function evaluarCableado({ archivos, adopto, distribuye, scaffoldCablea, ramaPorDefecto = "main" }) {
  ...
  if (invocaciones.length > 0) {
    hallazgos.push({
      ...
      residuo: true,
      mensaje: `RESIDUO DECLARADO (A01, modo aviso desde el 2026-08-21): la condicion 5 de este check —"un rojo de la compuerta impide que ${VEREDICTO_AGREGADO} salga verde"— promete una propiedad sobre un CAMINO (del job de la compuerta, por cada eslabon de needs, hasta el check run cuyo nombre exige el ruleset) y lo que verifica es un patron sintactico sobre un NODO. Medido con oraculo semantico independiente: 70 falsos verdes sobre 2928 casos generados, una sola clase, cuyo representante mas corto es un paso de ${VEREDICTO_AGREGADO} con "if: needs.<job>.result == success", que satisface la compuerta SALTEANDOSE. O sea: que este paso no reporte nada sobre el camino NO acredita que el rojo llegue al check requerido. Lo que si queda acreditado es todo lo demas: que la invocacion este rastreada, en el primer nivel de ${DIR_WORKFLOWS}, en modo verificar, disparada en el camino del cambio, no apagada ni amortiguada, y esperada por ${VEREDICTO_AGREGADO}. El diagnostico completo y el backlog viven en docs/08-reglas-no-escritas.md`,
    });
  }
...
const JOB_SUGERIDO = [
  ...
  `      - uses: im-diego-ec/Projects/${SEGMENTO_ACTION}@v1.7.0`,
  "        with:",
  "          modo: verificar",
  "",
