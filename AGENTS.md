...
**Herramientas verdes no bastan para specs que cambian contrato.** En el repo
de origen, un change pasó `validate --strict` y el guardrail de deltas en verde
...
- Declarar `permissions` explícitos en cada workflow, con el mínimo necesario, y
  ...
  (lección repetida tres veces en el repo de origen).
...
- **Sin acentos en los comentarios de YAML de los workflows** (convención
  heredada del repo de origen). La prosa de Markdown sí lleva acentos.
